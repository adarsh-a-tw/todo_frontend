...
describe("Basic Rendering of App component", () => {
    ...
    it("Displays Title", () => {
        const view = render(<App fetchAPI={mockFetchTodos} createAPI={mockCreateTodo} deleteAPI={mockDeleteTodo} editAPI={mockEditTodo}/>);
...
describe("Basic Functionality of App component",()=>{
    ...
    it("Edit Todo",()=>{
        const view = render(<App fetchAPI={mockFetchTodos}  createAPI={mockCreateTodo} deleteAPI={mockDeleteTodo} editAPI={mockEditTodo}/>);
        const data = "Hello World";
        view.getByTestId(`todo-edit-btn-2`).click();
        userEvent.type(view.getByTestId("update-box"), "Hello World");
        view.getByTestId('update-submit-btn').click();
        expect(view.getByTestId('todo-desc-2')).toHaveTextContent(data);
    });
})