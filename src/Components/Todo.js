...
const Todo = (props) => {
    return <li data-testid={"todo-div"} className="todo stack-small">
        <div className="c-cb">
            {props.completed ? <input type="checkbox" id={`todo-checkbox-${props.id}`} data-testid={`todo-checkbox-${props.id}`} checked readOnly/> : <input type="checkbox" data-testid={`todo-checkbox-${props.id}`} />}
